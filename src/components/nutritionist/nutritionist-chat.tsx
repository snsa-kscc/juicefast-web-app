...
export function NutritionistChat({ userId, userProfile = null, recentMeals = [] }: NutritionistChatProps) {
  // State for chat UI
      ...
      if (activeSession) {
        ...
        if (pendingRequests.length > 0) {
          ...
          if (pendingRequests[0].requestedNutritionistId) {
            const nutritionist = allNutritionists.find((n) => n.id === pendingRequests[0].requestedNutritionistId);
            if (nutritionist) {
              setSelectedNutritionist(nutritionist);
            }
          }
  ...
  const handleSubmitRequest = async () => {
    ...
    setIsLoading(true);
    try {
      await requestChatSession(userId, selectedNutritionist.id, initialQuestion);
        ...
        return (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="p-4 pb-2">
              <h3 className="text-lg font-medium mb-2">Available Nutritionists</h3>
            </div>
            <div className="flex-1 overflow-hidden">
              <ScrollArea className="h-[calc(100vh-360px)]" type="always">
      ...
      case "chatting":
        return (
          <div className="flex flex-col h-full p-4">
            {selectedNutritionist && (
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-medium text-sm">{selectedNutritionist.name}</div>
                    <div className="text-xs text-gray-500">{selectedNutritionist.specialties[0]}</div>
                  </div>
                </div>
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto mb-4 pr-2">
