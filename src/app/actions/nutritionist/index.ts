...
import { NutritionistProfile, ChatSession, AvailabilityStatus, SessionRequest, ChatNotification } from "@/types/nutritionist";
...
export async function acceptSessionRequest(requestId: string, nutritionistId: string): Promise<ChatSession | null> {
  ...
  if (request.initialQuery) {
    await dbActions.createChatMessage(session.id, request.initialQuery, "user", request.userId);
  }
...
export async function getNutritionistPendingSessionRequests(nutritionistId: string): Promise<SessionRequest[]> {
  return dbActions.getNutritionistPendingSessionRequests(nutritionistId);
}

export async function getChatMessages(sessionId: string): Promise<MessageType[]> {
  const messages = await dbActions.getChatMessages(sessionId);
  return messages;
}
...
// Direct Chat Session Creation (for client-side chat feature)
export async function createDirectChatSession(userId: string, nutritionistId: string, initialQuery?: string): Promise<ChatSession> {
  ...
  const session = await dbActions.createChatSession(userId, nutritionistId);

  // If there was an initial query, add it as the first message
  if (initialQuery) {
    await dbActions.createChatMessage(session.id, initialQuery, "user", userId);
  }
